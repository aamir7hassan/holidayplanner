		...
		<div class="container body">
			<div class="main_container">
          	...
          	<div class="col-md-12 col-sm-12 col-xs-12  maxi">
							<div class="x_panel">
								<div class="x_title">
									<h2>Checklisten <span class="text-approved">abgegeben</span></h2>
				...
				<div class="col-md-4">
                 <div class="form-group">
					<select class="form-control" id="cl_list">
					<?php  
					
					 
						  foreach($cl_list as $obj){ 
						  foreach($count as $co){
							  if($obj['sv_id']==$co['c_id']){?>
							  <option value='<?=$obj['sv_id']?>'><?=$obj['object_supervisor']?>(<?=$co['count']?>)</option>
                ...
                <div class="table-responsive">
                  <table class="table table-striped table-bordered tbl tablee">
                    ...
                    <tr>
                			...
                			<td class="text-center"><a href="<?=base_url('admin/checklisten/detail-view/'.$objects['id'])?>" data-toggle='tooltip' class="text-success" title="Details"><?
                				...
                				if ($avg>=5) {
                					...
                					$avg_color = " avg_yellow";
                				}
                				else {
                					$avg_color = " avg_green";
                				}

                				echo "<div class=\"avg" . $avg_color . "\">". number_format($avg, 2) . "</div>";
                				?></a>
                			</td>
					<?php }}?>
                    </tbody>
...
<!---end edit model--->
		...
		<script>
		 ...
		 $(document).ready(function(){
			$(document).on("change", '#cl_list', function(e) {
				 var id=$('#cl_list').val();
				 $.ajax({
					url:url,
					type:'POST',
                    dataType:'JSON',
                    data:{'req':'superviosr_data_checked','id':id},
                    success:function(res){
                      if(res.data)
					  {
						  console.log(res.data)
							  ...
							  $('.tablee tr').not(':first').remove();
							 
								$('.tablee tr').first().after(res.data);
