	...
	<body class="nav-md">
							...
							<div class="x_panel">
								<div class="x_title">
									...
									<ul class="nav navbar-right panel_toolbox">
										<li><a href="#" data-toggle="modal" data-target="#addmanager" class="btn btn-success" style="color:#fff">+ Objektmanager</a></li>
									</ul>
									<div class="clearfix"></div>
								</div>
								<div class="x_content">
		...
		<!-- / .body -->
		<div class="modal" id="addmanager"  role="dialog" tabindex="-1">
			<div class="modal-dialog modal-lg">
				<div class="modal-content">
					<div class="modal-header">
						<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">×</span>
						</button>
						<h4 class="modal-title" id="myModalLabel">Objektmanager hinzuf&uuml;gen</h4>
					</div>
					...
					<div class="modal-footer">
						...
						<button type="button" class="btn btn-primary" id="add">Objektmanager hinzuf&uuml;gen</button>
					</div>
				</div>
			</div>
		</div>
		
		<div class="modal fade editModal" id="editobjektmanager" tabindex="-1" role="dialog" aria-hidden="true">
			<div class="modal-dialog modal-lg">
				<div class="modal-content">
					<div class="modal-header">
						<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">×</span>
						</button>
						<h4 class="modal-title" id="myModalLabel">Objektmanager bearbeiten</h4>
					</div>
					<div class="modal-body">
						<form role="form">
		...
		<script>
			...
			$(document).on('click','#add',function(e) {
				...
				$.ajax({
					...
					type:"POST",
					dataType:"JSON",
					data:{'req':'add_objekt_manager','fname':fname,'lname':lname,'email':email},
					success:function(res) {
						if(res=="1") {alert('if');
							toastr.success('Objektmanager erfolgreich hinzugefügt.');
				...
				$.ajax({
					...
					success:function(res){
						if(res=="1") {
							...
							toastr.success('Objektmanager erfolgreich aktualisiert.');
							window.location.reload();
						} else {
							toastr.error(res);
						}
			...
			$(document).on('click','.activate',function(e){
				...
				$.ajax({
					...
					success:function(res){
						if(res=="1") {
							toastr.success('Objektmanager erfolgreich aktiviert.');
							window.location.reload();
						} else {
							toastr.error(res);
						}
			...
			function del(id)
			{
				...
				text: "Der Objektmanager wird gesperrt!",
				type: "warning",
				showCancelButton: true,
				confirmButtonColor: "#DD6B55",
				confirmButtonText: "Ja, sperre den Objektmanager!",
				customClass: "se_del",
				closeOnConfirm: true }, function(){
					$.ajax({
						...
						success:function(res) {
							if(res=="1"){
								toastr.success('Objektmanager erfolgreich gesperrt');
								window.location.reload();
							} else {
								toastr.error(res);
							}
