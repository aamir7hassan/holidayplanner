	...
	<body class="nav-md">
							...
							<div class="x_panel">
								<div class="x_title">
									...
									<ul class="nav navbar-right panel_toolbox">
										<li><a href="#" data-toggle="modal" data-target="#addModal" class="btn btn-success" style="color:#fff">+ Bereichsleiter</a></li>
									</ul>
									<div class="clearfix"></div>
								</div>
								<div class="x_content">
		...
		<!-- / .body -->
		<div class="modal" id="addModal"  role="dialog" tabindex="-1">
			<div class="modal-dialog modal-lg">
				<div class="modal-content">
					<div class="modal-header">
						<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">×</span>
						</button>
						<h4 class="modal-title" id="myModalLabel">Bereichsleiter hinzufügen</h4>
					</div>
					...
					<div class="modal-footer">
						...
						<button type="button" class="btn btn-primary" id="add">Bereichsleiter hinzufügen</button>
					</div>
				</div>
			</div>
		</div>
			...
			<div class="modal-dialog modal-lg">
				<div class="modal-content">
					<div class="modal-header">
						...
						<h4 class="modal-title" id="myModalLabel">Bereichsleiter bearbeiten</h4>
					</div>
					<div class="modal-body">
						<form role="form">
		...
		<script>
			...
			$(document).on('click','#add',function(e) {
				...
				$(this).html('Verarbeite Daten...');
				$.ajax({
					url:url,
					type:"POST",
					dataType:"JSON",
					data:{'req':'add_sv','fname':fname,'lname':lname,'email':email},
					success:function(res) {
						if(res=="1") {
							toastr.success('Bereichsleiter erfolgreich hinzugefügt.');
			...
			$(document).on('click','#update',function(e){
				...
				if(email==""){
					$('.email').html('Bitte die E-Mail-Adresse eintragen.');
					return false;
				}
				$(this).prop('disabled',true);
				$(this).html('Updating...');
				var frm = {'req':'process_edit_sv','fname':fname,'lname':lname,'pass':pass,'email':email,'id':id};
				$.ajax({
					url:url,
					type:"POST",
					dataType:"JSON",
					...
					success:function(res){
						if(res=="1") {
							...
							toastr.success('Bereichsleiter erfolgreich aktualisiert.');
							window.location.reload();
						} else {
							toastr.error(res);
						}
			...
			$(document).on('click','.activate',function(e){
				...
				if(id==""){
					toastr.error('Es gibt ein Problem, bitte die Seite neu laden.');
					return false;
				}
				$(this).prop('disabled',true);
				$(this).html('Activating...');
				var frm = {'req':'activate_sv','id':id};
				$.ajax({
					url:url,
					type:"POST",
					dataType:"JSON",
					...
					success:function(res){
						if(res=="1") {
							toastr.success('Bereichsleiter erfolgreich aktiviert.');
							window.location.reload();
						} else {
							toastr.error(res);
						}
			...
			function del(id) {
				...
				text:  "Der Bereichsleiter wird gesperrt!",
				type:  "warning",
				showCancelButton: true,
				confirmButtonColor: "#DD6B55",
				confirmButtonText: "Ja, sperre den Bereichsleiter!",
				customClass: "se_del",
				closeOnConfirm: true }, function(){
					$.ajax({
						...
						type:"POST",
						dataType:"JSON",
						data:{'req':'delete_sv','id':id},
						success:function(res) {
							if(res=="1"){
								toastr.success('Bereichsleiter erfolgreich gesperrt');
								window.location.reload();
							} else {
								toastr.error(res);
							}
